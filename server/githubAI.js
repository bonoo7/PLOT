...
استراتيجيتك: استبدل الفراغات بأي كلمات تبدو معقولة ومنطقية.`;
    ...
    const prompt = `أنت لاعب في لعبة "الحبكة" الاجتماعية.

القضية: "${scenario.title}"

الجملة الأصلية (فيها فراغات _____):
"${template}"

${roleInstruction}

⚠️ القواعد الصارمة:
1. أعد كتابة الجملة كاملة كما هي، واستبدل كل _____ بكلمة أو عبارة قصيرة مناسبة فقط.
2. لا تغيّر ترتيب الكلمات الأخرى أو تعيد صياغة الجملة.
3. لا تضف مقدمات أو شرحاً أو علامات اقتباس.
4. اكتب بالعربية من اليمين لليسار فقط.
5. الرد يجب أن يكون الجملة المكتملة فقط — لا أكثر.
6. تأكد أن إجابتك مختلفة عن: ${scenario._prevAnswers || 'لا يوجد'}.

الجملة المكتملة (بدون علامات اقتباس):`;
