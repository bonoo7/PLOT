...
### بيئة الذكاء الاصطناعي للبوتات وإكمال الفراغ الحصري (Smart Bot AI & Blitz-Only Mode) 🧩🧠🤖

#### 1. إكمال الفراغ كنمط اللعب الوحيد (Blitz-Only Game Mode)
- **الخادم (Server)**:
  - إزالة نمط الكتابة الكاملة (Classic Mode) نهائياً من كافة مراحل اللعب وعواقب القدرات.
  - إرسال قوالب الفراغات تلقائياً لجميع شاشات العميل في بدء الصياغة والتصويت والنتائج.
  - قصر قدرة الشاهد وقدرة العراف على الفراغات بنسب دقة متناقصة.
  - تعيين نمط اللعب ليكون `BLITZ` تلقائياً عند إنشاء غرفة وتعديل إعداداتها، وإلغاء أي معامل آخر.
- **العميل (plot-mobile)**:
  - إزالة واجهة اختيار الأنماط (تغيير النمط) نهائياً من شاشات المضيف في V3 و V2 و Legacy.
  - تبسيط واجهات الصياغة (Drafting Screens) في V3 و V2 و Legacy لإظهار واجهة ملء الفراغات حصرياً وإلغاء حقول الكتابة الحرة.

#### 2. تطوير بيئة الذكاء الاصطناعي الاستراتيجية للبوتات (Smart Bot AI Environment)
- **الكتابة الاستراتيجية التخصصية للبوتات**:
  - صياغة السيناريوهات وملء الفراغات تختلف باختلاف هوية البوت ودوره لخدمة فريقه:
    - *الجاني البوت*: يملأ الفراغات بالسيناريو الحقيقي تلقائياً لضمان الدقة وتجنب كشفه.
    - *المخرب البوت*: يضع الكلمات المخادعة لتشتيت الانتباه.
    - *العقل المدبر البوت*: يكتب سيناريوهات مضللة تشتت الانتباه عن فريقه وتُبرز شكوكاً في الأبرياء.
    - *المستفيد البوت*: يكتب بأسلوب يقلل من الجريمة ويركز على النواحي المادية للتمويه.
    - *العراف البوت*: ينسخ القصة الحقيقية تلقائياً بنسبة 90% (قدرة الوحي).
- **التصويت والقدرات الاستراتيجية**:
  - *توجيه القدرات*: المحقق البوت يوجه التحقيق للاعب الأكثر شكاً، والمخرب البوت يستشرف هدف المحقق لقلب وتخريب النتيجة.
  - *التصويت على الجاني*: الجاني البوت يضلل التصويت لخدمة فريقه، والمستفيد البوت يحمي شركاءه، والعدالة تصوت وفق الأدلة وحجم المعلومات المتوفرة لها.
- **العروض والرشاوى الذكية وتفادي فخ الوزير**:
  ...
  - البوت الوزير يستقبل العروض السرية ويفعّل تنبيه الكشف المباشر في حال استهدافه، ولديه القدرة الكاملة على تقديم عروض سرية ذكية للمساعدة في كشف الحقائق.

#### 3. استقرار واختبارات النظام
- **الاختبارات**: نجاح جميع الاختبارات التلقائية (22/22) بنسبة 100%.
...
### إدارة البوتات يدوياً، منع تكرار أدوار التدريب، وإصلاحات الاستقرار 🤖🛠️

#### 1. إدارة البوتات يدوياً من قبل المضيف (Host Bot Console)
- **إضافة البوتات يدوياً**: استبدال الإضافة العشوائية المباشرة للبوتات بلوحة اختيار دور البوت (جاني، شاهد، محقق، إلخ، أو عشوائي) تظهر عند الضغط على زر `+ BOT` في شاشة الانتظار للمضيف.
- **إزالة البوتات يدوياً**: إضافة زر `[- حذف]` باللون الأحمر بجانب شارة كل بوت في قائمة المضيف لإمكانية طرد البوتات يدوياً عبر حدث `removeBot`.
- **إعادة الفهرسة والترتيب تلقائياً**: بناء دالة `reindexBots` لإعادة ترقيم وتسمية البوتات المتبقية ديناميكياً (مثل `Bot 1 🤖 (المحقق)`) عند حدوث أي إضافة أو حذف.

#### 2. إصلاح تكرار البوتات وتداخل الأدوار في التدريب (Training Bot Duplicate Fix)
- **تهيئة أول لاعب**: جعل أول لاعب ينضم للتدريب فقط هو من يقوم بإنشاء البوتات الافتراضية الثلاثة لتكملة الحد الأدنى للعب (4 لاعبين).
- **منع التكرار للاعبين اللاحقين**: عند دخول لاعب ثانٍ أو ثالث للتدريب، يمتنع السيرفر عن إنشاء أي بوتات إضافية.
- **تصفية تعارض الأدوار**: يقوم السيرفر تلقائياً بإزالة البوت الذي يطابق دور اللاعب المنضم حديثاً لمنع ازدواجية الأدوار في الغرفة، ثم يعيد فهرسة البوتات المتبقية.
- **تثبيت دور التدريب**: إصلاح مشكلة عدم قراءة الدور المفضل للاعبين الجدد في غرف التدريب (عن طريق تصحيح شرط التحقق من `desiredRole !== undefined || room.isTutorial`) وتأكيد تخصيص دورهم المختار بدلاً من تحويلهم للوضع العشوائي.

#### 3. قصر عواقب قبول العروض وتصفير المؤقتات
- **دورة حياة العروض**: تعديل منطق السيرفر في بداية كل جولة جديدة ليقوم بتصفير العرض المقبول `player.acceptedOffer = null` لجميع اللاعبين، مما يضمن استعادة اللاعب لقدراته الخاصة بعد جولة واحدة فقط من قبوله للرشوة.
- **تصفير مؤقت النقاش**: إزالة العداد المتجمد من شاشة النقاش للحد من تشتيت اللاعبين.

#### 4. اختبارات النظام واستقرار البناء
