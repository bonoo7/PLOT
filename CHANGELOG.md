...
### إطلاق نظام التصميم V3 "Retro Terminal" وإصلاحات الجودة 🖥️💚

#### 1. نظام التصميم V3 المتكامل (Retro Terminal Style)
- **الهوية البصرية**: بناء نظام تصميم أحادي اللون (Monochrome) بنمط الشاشات المحطية القديمة (Terminal Retro).
- **المكونات الجديدة**: إضافة 12 مكوناً جديداً تحت مجلد `design-v3/components/`:
  - `TerminalLayout.js`: التخطيط الثلاثي المرن والمتوافق مع SafeArea.
  - `TerminalHeader.js` و `TerminalCard.js`: الأشرطة العلوية وحاويات العرض الكلاسيكية.
  - `TerminalButton.js` و `TerminalInput.js`: عناصر الإدخال والأزرار التفاعلية.
  - `GlitchText.js` و `BlinkCursor.js` و `ScanLines.js`: تأثيرات النصوص الملتوية والمؤشر الوامض وخطوط المسح الشاشية.
  - `ProgressBar.js` و `TerminalBanner.js`: مؤشرات التقدم واللافتات التحذيرية.
- **الشاشات المحسنة**: إعادة صياغة جميع شاشات اللعب الـ 10 تحت `design-v3/screens/` لتتوافق مع نظام التشغيل الكلاسيكي الافتراضي للعبة.
- **معالجة الأخطاء**: إضافة مكون `ErrorBoundary.js` بأسلوب شاشات الخطأ المحاكية لأنظمة DOS لتفادي الانهيارات المفاجئة.

#### 2. تحسين تمييز نصوص البلتز (Blitz Mode Word Highlights)
- **الإصلاح**: تم تضمين حقل القالب `template` في بيانات السيناريوهات المرسلة من السيرفر في أحداث `qualityVotingStarted` و `culpritVotingStarted`.
- **دعم إعادة الاتصال**: إرفاق حقل `template` في السيناريوهات أثناء إعادة انضمام المضيف (`rejoinHost`) أو اللاعب (`joinRoom`) في مرحلتي التصويت.
- **النتيجة**: تلوين الكلمات التي تمت صياغتها باللون الأصفر الفاقع المضيء (`#FFFF00`) بشكل ديناميكي ومستمر على شاشات التصويت للاعبين والمضيف.

#### 3. حماية سرية الأدوار في شاشة النتائج (النتائج غير الحاسمة)
- **المشكلة**: كان جدول نتائج الجولة يسمح بتوسيع صفوف اللاعبين لمشاهدة تفصيل النقاط (Breakdown) والتحالفات حتى لو لم ينتهِ اللعب تماماً، مما يفضح الأدوار والقدرات السرية.
- **الإصلاح**: إضافة خاصية `hideDetails` لمكون `ResultCard` لتعطيل كشف تفاصيل توزيع النقاط ومنع تمدد الأكورديون نهائياً.
- **التفعيل**: تمرير `hideDetails` بناءً على فحص حالة الفوز للجولة (إذا كانت `winner === 'CONTINUE'`). تم التفعيل في شاشة نتائج اللاعب (`PlayerResultsScreen`) وشاشة نتائج المضيف (`HostResultsScreen`).

#### 4. إعادة هيكلة شاشة نقاش المضيف (Host Discussion Screen Redesign)
- **تحديث التخطيط**: تقديم السيناريوهات والتلميحات بشكل متجاور (Side-by-side) بدلاً من البنية الرأسية المستهلكة للمساحة.
- **التحكم بالعرض**: إضافة أزرار مخصصة لإخفاء أو إظهار العناصر التوضيحية وتلميحات اللاعبين بشكل مرن.
- **إبراز المتحدث النشط**: جعل حاوية المتحدث الحالي أكبر بصرياً وتضخيم موجات الحركة الصوتية المرافقة (Soundwave Scale) لتوفير دلالة بصرية أوضح للمستمعين.

#### 5. استقرار السيرفر وبناء التطبيق
- **أتمتة التطوير**: تزويد سكريبت التشغيل `start_fixed.js` بنظام مراقبة الملفات (File Watcher) لإعادة البناء التلقائي عند تعديل أي ملف في `src/`.
- **منع اختناق السيرفر**:
  - حل مشكلة القفل الميت للاتصال بالـ DB عبر استخراج دالة الكتابة المباشرة `writeDBDirect` لتجنب التكرار في الحلقات المغلقة.
  - إيقاف تسريب الذاكرة في مؤقتات الغرف عبر تنظيفها بـ `safeDeleteRoom` قبل التخلص من الغرفة.
  - تنظيف النصوص والرموز التعسفية والـ UTF الملتوية من أسماء اللاعبين لمنع انهيار الترميز في قاعدة البيانات.
- **تحديث البناء**: توليد حزمة الويب وتجميعها بنجاح للإنتاج.

---

## [3.6.0] - 2026-03-13

### نظام الصوت البرمجي + إصلاحات الاتصال والواجهة 🎵🔧

#### 1. موسيقى تعبيرية مولَّدة برمجياً (procedural audio)
- إنشاء 6 مقاطع صوتية `.wav` قابلة للتكرار مولَّدة بالكامل بـ Node.js بدون أصول خارجية
- كل مرحلة من اللعبة تحمل موسيقاها المناسبة:
  - `opening_loop.wav` — شاشة البداية، اللوبي، بدء الجولة، الكتابة (طابع هادئ مستمر)
  - `discussion_loop.wav` — مرحلة النقاش (توتر هادئ)
  - `quality_voting_loop.wav` — تصويت الجودة (مثير ومعتدل)
  - `culprit_voting_loop.wav` — تصويت الجاني (توتر متصاعد هادئ)
  - `dramatic_reveal_loop.wav` — الكشف الدرامي (غامض وعميق)
  - `results_loop.wav` — شاشة النتائج (كشف الحقيقة)

#### 2. إعادة هيكلة `soundManager.js`
- فصل "اسم المرحلة" عن "هوية التراك" عبر جدول `STAGE_TRACKS`
- عدة مراحل تشترك في نفس التراك → لا إعادة تشغيل عند التنقل بينها
- تسلسل آمن للعمليات عبر `queueMusicTransition` يمنع تشغيل مقطعين في آن واحد
- إصلاح `setSoundEnabled(false)` ليُوقف الصوت فعلياً قبل الإرجاع

#### 3. موسيقى شاشة القائمة الرئيسية
- إضافة `playMusic('lobby')` في كلا نسختَي `RoleSelectScreen` (V1 و V2)
- الموسيقى تستمر دون انقطاع عند الانتقال: القائمة ← إنشاء غرفة ← اللوبي ← بدء الجولة ← الكتابة

#### 4. إصلاح اتصال WebSocket في وضع التطوير
- الكود كان يستخدم `window.location.origin` دائماً في المتصفح فيوجّه الاتصال لـ Expo dev server (`:8081`) بدل سيرفر اللعبة (`:3000`)
- الإصلاح: وضع التطوير (`__DEV__`) يستخدم `DEV_SERVER_IP:DEV_SERVER_PORT` مباشرةً، والإنتاج يبقى على `window.location.origin`

#### 5. إصلاح خطأ "text node cannot be a child of View" في شاشة التصويت
- نص تعادل التصويت كان يُمرَّر كـ JSX مختلط (نص + تعبير) لـ `ClassifiedBanner` فيصبح array لا string
- الإصلاح في `HostGameScreens.js`: تغليف النص بـ `<Text>` صريح
- تحسين `ClassifiedBanner` ليتعامل تلقائياً مع مصفوفات النصوص ويغلّفها بـ `<Text>`

#### 6. تنظيف الأصول الصوتية
- حذف 9 ملفات `.mp3` غير مستخدمة من `plot-mobile/assets/sounds/`
- تحديث `README.md` في مجلد الأصوات الصوتية

---

## [3.5.0] - 2026-03-08

### سيناريوهات موسّعة + إصلاح الجولات + تنظيف المكتبات 🎭

#### 1. توسيع السيناريوهات (59 قضية)
- إضافة 35 سيناريو جديد (IDs 25–59) بمواضيع: مكاتب، رياضة، علوم، سفر، حيوانات، طعام، سياسة، تكنولوجيا
- إصلاح `keywords` في جميع السيناريوهات الـ 24 الأصلية لتكون تلميحات غير مباشرة لا مطابقة للإجابات
- التحقق الآلي: لا يوجد keyword يطابق أي blank

#### 2. إصلاح تكرار السيناريوهات
- وضع التدريب كان يُعيد السيناريو 1 في كل جولة — تم الإصلاح
- الآن كلا الوضعين (تدريب + عادي) يختاران سيناريو عشوائياً من غير المستخدمة

#### 3. تنظيف package.json
- root: إزالة `puppeteer`, `qrcode`, `expo` غير المستخدمة؛ إضافة سكريبت `install:all`
- plot-mobile: نقل `@types/react` لـ devDependencies فقط؛ إزالة تكرار `typescript`
