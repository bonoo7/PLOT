...
## حالة المشروع الحالية (الإصدار 4.0.0)

### آخر التحديثات (2026-05-24 — v4.0.0 — بيئة الذكاء الاصطناعي للبوتات وإكمال الفراغ الحصري):
- ✅ **تطوير بيئة الذكاء الاصطناعي للبوتات (Smart Bot AI)**:
  - صياغة السيناريوهات وملء الفراغات ديناميكياً بناءً على هوية البوت ودوره لخدمة فريقه (الجاني يملأ بالقصة الحقيقية، المخرب يضع كلمة الخداع، المستفيد يوجه المصلحة، العراف ينسخ بالوحي).
  - تصويت استراتيجي وقدرات ذكية مبنية على الفرق وعزل المعلومات المتاحة (المحقق يفحص الأكثر شكاً، المخرب يوجه التخريب بدقة، العقل المدبر يحمي أعضاء فريقه).
  - تفاعل ذكي مع العروض السرية والرشاوى وتفادي الفخاخ (المستفيد يتجنب رشوة الوزير حماية لنفسه، والوزير يفعل تنبيه الكشف الفوري ويقدم عروضاً ذكية).
- ✅ **إكمال الفراغ كنمط اللعب الوحيد (Blitz-Only Mode)**:
  - إزالة نمط الكتابة الكاملة (Classic Mode) نهائياً من كافة مراحل اللعب بالخادم وشاشات تطبيق الجوال.
  - إزالة أزرار وواجهات اختيار وتحديث نمط اللعب في شاشات المضيف لثيم V3 و V2 و Legacy.
  - قصر واجهات الصياغة للتقارير (Drafting Screens) على إظهار نموذج الفراغات أو تحميلها حصرياً، وإلغاء حقول إدخال النص الحر تماماً.
- ✅ **استقرار واختبارات كاملة**: نجاح كافة الاختبارات التلقائية الـ 22 في السيرفر وبناء حزمة الويب وتصديرها بنجاح تام.

---

## نظام التصميم V3 "Retro Terminal" — التوثيق الكامل

### البنية المعمارية

#### 1. التخطيط الثلاثي للترمينال (`TerminalLayout.js`)
تم تصميم الواجهة بنظام الحاويات الثلاثية المحمية لمنع تجاوز حدود الشاشة وتثبيت الأشرطة والأزرار:
```
SafeAreaView [root] — flex: 1, backgroundColor: c.bg (أسود داكن #030712)
  └── ScanLines [Overlay] — خطوط مسح شاشة CRT شبه شفافة تعلو كامل الشاشة
        └── View [inner] — flex: 1, width: '100%', maxWidth: contentMaxW, alignSelf: 'center'
              ├── View [topZone]    — TerminalHeader: كود الغرفة / اسم القضية / بيانات المضيف واللاعبين
              ├── View [centerZone] — محتوى الشاشة المتغير والنشط (flex: 1) مع دعم التمرير الداخلي الآمن
              └── View [bottomZone] — أزرار التحكم الرئيسية (TerminalButton) الملتصقة بالأسفل
```

#### 2. الألوان وتوكينات الثيم الفسفوري (`tokens/index.js`)
يعتمد الثيم V3 على لوحة ألوان أحادية اللون مستوحاة من الحواسيب الكلاسيكية:
- **الخلفية (`bg`)**: أسود داكن جداً `#030712` مع لون حاوية وسيط `#111827` (Slate 900).
- **اللون الرئيسي للفسفور (`primary`)**: أخضر ترمينال زاهٍ `#00FF00` أو برتقالي أمبر `#F59E0B` حسب نوع الشاشة أو الوضع.
- **التلميحات والتمييز (`accent`)**: لون الكلمات المكتملة في Blitz أو الكلمات الهامة بالأصفر المضيء `#FFFF00`.
- **الحدود والأطر (`border`)**: حدود برمجية رفيعة بسمك `1px` بلون أخضر ترمينال شبه شفاف `#00FF0044` تعطي شكل الجداول القديمة.

#### 3. المكونات الأساسية المتكاملة
- **`TerminalTypewriter.js`**: مكوّن يستقبل النص ويقوم بطباعته حرفاً بحرف بسرعة تحاكي طباعة الترمينال القديم مع خيار تشغيل/إيقاف المؤشر الوامض عند الانتهاء.
- **`GlitchText.js`**: نص تفاعلي يهتز ويتحرك بشكل عشوائي خفيف بأسلوب خطأ الترمينال (Software Glitch) لتسليط الضوء على الكلمات والملفات الحساسة.
- **`BlinkCursor.js`**: مؤشر ترمينال مستطيل وامض (`▋`) يعمل على نبضات أنيميشن دورية.
- **`ScanLines.js`**: طبقة overlay شفافة توزع خطوطاً أفقية متكررة تحاكي زجاج شاشات الكاثود (CRT).
- **`TerminalButton.js`**: أزرار ترمينال بحدود مربعة حادة وتأثير وميض عند الضغط، مع دعم للوضع المعطل بصرياً بألوان باهتة.
- **`TerminalInput.js`**: حقول إدخال بنصوص خضراء/أمبر ومؤشرات كتابة ترمينال متكاملة وتخصيص كامل للوحة المفاتيح.
- **`PlayerBadge.js`**: شارة موحدة لعرض اسم اللاعب وأدائه وصورته الرمزية في قوائم اللوبي والتصويت مع مؤشر خاص بالمضيف (`isHost`).
- **`ResultCard.js`**: بطاقة نتائج الجولة تدعم كشف النقاط التدريجي وخاصية الحماية `hideDetails`.

#### 4. تلوين نصوص البلتز في شاشات التصويت والجاني
في شاشات تصويت الجودة وتصويت الجاني، تتم مقارنة نص الإجابة المصاغ مع قالب القضية الأصلي `template` عبر محرك `renderBlitzText`.
لتلوين الكلمات المدخلة باللون الأصفر `#FFFF00` وتثبيتها بشكل مستمر:
- السيرفر يقوم بإرسال حقل الـ `template` ضمن محتوى السيناريو في حدث `qualityVotingStarted` و `culpritVotingStarted`.
- يُبث القالب أيضاً أثناء معالجة إعادة الانضمام للمضيف واللاعب لضمان ثبات اللون المضيء حتى بعد انقطاع الاتصال.

#### 5. منع كشف الأدوار (Round Results Security)
لمنع تسرب أدوار اللاعبين السرية خلال الجولات المستمرة (`winner === 'CONTINUE'`)، تم تقييد تفاصيل لوحة النتائج:
- يستقبل مكوّن `ResultCard.js` خاصية `hideDetails={true}`.
- عند تفعيلها، يُعطّل كود الأكورديون التمدد ولا يعرض توزيع نقاط "تحالف الجريمة" أو "تأثير القدرات الخاصة" أو "نقاط التصويت"، ويكتفي بعرض المجموع العام فقط.

#### 6. إعادة تصميم شاشة نقاش المضيف
لتسهيل قراءة مجريات اللعب على شاشة المضيف الكبيرة:
- تم تقسيم الشاشة أفقياً إلى لوحتين متجاورتين: لوحة السيناريوهات المكتملة يميناً، ولوحة التلميحات والكلمات المفتاحية يساراً.
- تزويد المضيف بأزرار سريعة لإظهار/إخفاء اللوحات لتقليل الفوضى البصرية.
- إبراز المتحدث الحالي بتكبير حجم البطاقة الخاصة به بمقدار 1.1x، ورفع مقياس أنيميشن الموجات الصوتية (`Soundwave`) لتقديم تمثيل مرئي واضح وممتع للحديث.
